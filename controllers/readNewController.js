...
const { FIND_ALL_ARTICLES_CRAWL, COUNT_TOTAL_ARTICLE_CRAWL, ERROR_CODE, INSER_ARTICLE_CRAWL, FIND_AUDIO_ARTICLE_CRAWL } = constants;
...
exports.createAudioArticle = async (req, res) => {
  ...
  const contentGroup = [];
  const audioGroup = ['https://static.openfpt.vn/text2speech-v5/short/2020-06-17/banmai.0.47cfbfdc06230074ecec599773067e0d.mp3'];
    ...
    request
      ...
      .end((err, data) => {
        ...
        if (i === contentGroup.length - 1) {
          const request = new sql.Request();
          request.query(INSER_ARTICLE_CRAWL.replace('ArticleIdValue', id).replace('AudioUrlValue', audioGroup.toString()), (err) => {
            if (err) {
              res.status(500).send();
            }
            res.json({
...
exports.getAudioArticle = (req, res) => {
  ...
  request.query(FIND_AUDIO_ARTICLE_CRAWL.replace('IdValue', id), (err, data) => {
    if (err) {
      res.statusCode = 500;
      res.json(err);
    }
